...
const PopularShop = () => {
  const FilteredShops = Shops.filter((shop) => shop.popular == true);
  const PopularShops = FilteredShops.filter((shop, index) => index < 6);
  return (
    <div className="row">
      {PopularShops.map((shop) => (
        <div className="col-md-6" key={shop.id}>
          <div
            className={`popular_img ${shop.popular == true ? "active" : null}`}
          ></div>
